...
const PropertyModal: React.FC<PropertyModalProps> = ({ property, onClose, handleNavigate }) => {
    ...
    return (
        <section className="fixed inset-0 flex items-center justify-center w-full bg-black bg-opacity-50 z-50">
            <div className="relative bg-white flex flex-col md:flex-row gap-5 rounded-lg shadow-lg p-3 md:p-6 h-[70vh] md:h-[70vh] w-[80%] overflow-y-auto overflow-x-hidden">
                {/* property image  */}
                ...
                <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="flex justify-between items-start">
                        <h2 className="text-xl font-bold  text-gray-700">{property.property_name}</h2>
                    </div>
                    <hr />
                    {/* description  */}
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-row gap-1 md:gap-3">
                            ...
                            <p className="text-gray-600 text-sm md:text-base">: {property.property_location}</p>
                        </div>
                        <hr />
                        {/* amenities  */}
                        <div className="flex flex-col md:flex-row gap-3 items-start">
                            ...
                            <ul className="list-disc list-inside grid grid-cols-2 gap-4 ">
                                {property.property_amenities.map((amenity: any, index: any) => (
                                    <div className="flex items-center gap-3 text-gray-700" key={index}>
